...
namespace media
{
    ...
    HRESULT MediaFoundationStreamWrapper::RuntimeClassInitialize(uint32_t sourceStreamId, uint32_t mediaStreamId, GUID streamType, IMFMediaSource* parentSource, SampleGenerator *p_pSampleGenerator, IMFSourceReader* sourceReader)
    {
        m_streamType = streamType;
        m_sourceStreamId = sourceStreamId;
        m_mediaStreamId = mediaStreamId;
        m_sourceReader = sourceReader;
        m_pSampleGenerator = p_pSampleGenerator;
    ...
    HRESULT MediaFoundationStreamWrapper::RequestSample(IUnknown* token) 
    {
        ...
        RETURN_IF_FAILED(m_pSampleGenerator->ReadSample((DWORD)m_sourceStreamId,
                                                        &m_streamType,
                                                        0,
                                                        &actualStreamIndex,
                                                        &flags,
        ...
        //richfr todo figure out on calls where I use.Get() from a comptr.That does put a ref on the object being passed to the function
        //is it the function that reeceived this objects responsibility to release() the passed in object or will that happen automatically by the 
        //comptr class when this function returns? I assume the called function needs to do that and I don't so look into it.

        //richfr01 RETURN_IF_FAILED(m_sourceReader->ReadSample((DWORD)m_sourceStreamId, 0, &actualStreamIndex, &flags, &timestamp, &sample));
    ...
    {
        m_pSampleGenerator->GetCurrentMediaType(m_sourceStreamId, mediaTypeOut);//richfr01

        //richfr01 m_sourceReader->GetCurrentMediaType(m_sourceStreamId, mediaTypeOut);
        return S_OK;
    }
