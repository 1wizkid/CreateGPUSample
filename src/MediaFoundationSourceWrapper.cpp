...
namespace media
{
    ...
    HRESULT MediaFoundationSourceWrapper::RuntimeClassInitialize(SampleGenerator* p_pSampleGenerator, IMFSourceReader* sourceReader)
    {
        ...
        {
            validStream = false;
            HRESULT hr = m_pSampleGenerator->GetStreamSelection(streamIndexIn, &validStream);//richfr01
            //HRESULT hr = m_sourceReader->GetStreamSelection(streamIndexIn, &validStream);//richfr01
            ...
            if (validStream)
            {
                ...
                RETURN_IF_FAILED(m_pSampleGenerator->GetCurrentMediaType(streamIndexIn, &spMT));//richfr01
                //richfr01 RETURN_IF_FAILED(m_sourceReader->GetCurrentMediaType(streamIndexIn, &spMT));
                GUID guidMajorType;
                RETURN_IF_FAILED(spMT->GetMajorType(&guidMajorType));
                if(guidMajorType == MFMediaType_Audio)
                {
                    ComPtr<MediaFoundationStreamWrapper> stream;
                    RETURN_IF_FAILED(MakeAndInitialize<MediaFoundationStreamWrapper>(&stream, streamIndexIn, mediaStreamIndex, MFMediaType_Audio, this, p_pSampleGenerator, sourceReader));
                ...
                {
                    ComPtr<MediaFoundationStreamWrapper> stream;
                    RETURN_IF_FAILED(MakeAndInitialize<MediaFoundationStreamWrapper>(&stream, streamIndexIn, mediaStreamIndex, MFMediaType_Video, this, p_pSampleGenerator, sourceReader));
                    m_mediaStreams.push_back(stream);
                    mediaStreamIndex++;
                }    
            }
