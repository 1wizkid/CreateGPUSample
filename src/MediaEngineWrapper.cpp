...
{
    // Public methods
    HRESULT MediaEngineWrapper::RuntimeClassInitialize(SampleGenerator *p_pSampleGenerator, IMFSourceReader* sourceReader,
        ...
        uint32_t width, uint32_t height)
    {
        ...
        RunSyncInMTA([&]() {
            HRESULT hr = CreateMediaEngine(m_pSampleGenerator, sourceReader);//richfr01
            if (S_OK != hr)
    ...
    void MediaEngineWrapper::StartPlayingFrom(uint64_t timeStamp)
    {
        RunSyncInMTA([&]() {
            ...
            m_pSampleGenerator->SetCurrentPosition(GUID_NULL, startTime);//richfr01
            //richfr01 m_sourceReader->SetCurrentPosition(GUID_NULL, startTime);
    ...
    // Internal methods
    HRESULT MediaEngineWrapper::CreateMediaEngine(SampleGenerator *p_pSampleGenerator, IMFSourceReader* sourceReader)
    {
        ...
        RETURN_IF_FAILED(MakeAndInitialize<MediaFoundationSourceWrapper>(&m_source, p_pSampleGenerator, sourceReader));//richfr01
        m_sourceReader = sourceReader;
        m_pSampleGenerator = p_pSampleGenerator;//richfr01
